...
# plot
plot(turb2, type="l")
lines(turb2,type="l", col="red")

# variable contributions
library(timetk)
library(janitor)
turb.grad %>% dplyr::filter(Date>="2007-01-01", Date<="2015-12-01") %>% group_by(year = lubridate::floor_date(Date, "year")) %>% 
  summarise(across(-Date,~sum(.))) %>% adorn_totals("row") %>% as_tibble()
turb.GW %>% dplyr::filter(Date>="2007-01-01", Date<="2015-12-01") %>% group_by(year = lubridate::floor_date(Date, "year")) %>% 
  summarise(across(-Date,~sum(.))) %>% adorn_totals("row") %>% as_tibble()

# raw yield plot
INT_monthly %>% dplyr::select(-Member) %>% dplyr::filter(Date>="1999-01-01")%>% 
  pivot_wider(names_from="Country", values_from = "Yield") %>% dplyr::select(GR2M,POR10M) %>%
  scale()%>%
  ts(.,start=c(1999,1),frequency = 12) %>%
  ts.plot()

##################################
# EA (accounted for membership)
##################################

turb1 <- ISturbulence(X=input, GW=TRUE, norm=TRUE, weights=weights)
turb2 <- turb1$turb %>% as_tibble() %>% add_column(Date=as.Date(Dates)) %>% dplyr::select(Date, turb)
turb.grad <- turb1$turb.grad %>% as_tibble() %>% add_column(Date=as.Date(Dates))

plot(turb2, type="l")
turb.grad %>% dplyr::filter(Date>="2008-01-01", Date<="2014-12-01") %>% dplyr::select(-Date) %>% colSums(.,na.rm=TRUE)
...
CH <- INT_monthly %>% dplyr::select(-Member) %>% dplyr::filter(Date>="1999-01-01")%>% 
  pivot_wider(names_from="Country", values_from = "Yield") %>% 
  dplyr::select(Date,CH10M,	CH2M)

Dates <- as.yearmon(CH$Date)
input <- CH %>% dplyr::select(-Date) %>% xts(., order.by=Dates) 
turb1 <- ISturbulence(X=input, GW=TRUE, norm=TRUE)
turb2 <- turb1$turb %>% as_tibble() %>% add_column(Date=as.Date(Dates)) %>% dplyr::select(Date, turb)
turb.grad <- turb1$turb.grad %>% as_tibble() %>% add_column(Date=as.Date(Dates))
turb.GW <- turb1$turb.GW %>% as_tibble() %>% add_column(Date=as.Date(Dates))
