#########################################################################
# FROM S. STÖCKL (2019): https://github.com/sstoeckl/uncertaintymeasures
#########################################################################
...
#------------------------------------------------------------------------------------------------------------------
#########################################################################
# ----------- NOT NEEDED FROM HERE ON (OLD CODE SNIPPETS) ---------------
#########################################################################
...
library(timetk)
library(janitor)
turb.grad %>% dplyr::filter(Date>="1995-01-01") %>% mutate(across(-Date,~replace_na(., mean(.,na.rm=TRUE))))%>% 
  dplyr::filter(Date>="2007-01-01", Date<="2015-12-01") %>%
  group_by(year = lubridate::floor_date(Date, "year")) %>% 
  summarise(across(-Date,~sum(.))) %>% adorn_totals("row") %>% as_tibble()
turb.GW %>% dplyr::filter(Date>="1995-01-01") %>% mutate(across(-Date,~as.numeric(replace_na(., mean(.,na.rm=TRUE)))))%>% 
  dplyr::filter(Date>="2007-01-01", Date<="2015-12-01") %>%
  group_by(year = lubridate::floor_date(Date, "year")) %>% 
  summarise(across(-Date,~sum(.))) %>% adorn_totals("row") %>% as_tibble()

# WORKS GOOD:
# OSTurb, EA12, oLUX, roll=T, roll=60, equal weights, start = 1999-01-01
# OSTurb, EA12, oLUX, roll=T, roll=60, equal weights, start = 1994-01-01 (small breaks between 2003-2004)
# OSTurb, EA12, oLUX, roll=T, roll=60, equal weights, start = 1990-01-01 (small breaks between 1997-1998, 2003-2004)



##################################
# EA all (accounted for membership)
##################################

turb1 <- OSturbulence(X=input, norm=TRUE, GW=TRUE, weights=weights, rolling=TRUE, roll.obs=60, use="pairwise.complete.obs")
turb2 <- turb1$turb %>% as_tibble() %>% add_column(Date=as.Date(Dates)) %>% dplyr::select(Date, turb)
turb.grad <- turb1$turb.grad %>% as_tibble() %>% add_column(Date=as.Date(Dates))
turb.GW <- turb1$turb.GW %>% as_tibble() %>% add_column(Date=as.Date(Dates))
...
# Switzerland
##################################
CH <- INT_monthly %>% dplyr::select(-Member) %>% dplyr::filter(Date>="1990-01-01")%>% 
  pivot_wider(names_from="Country", values_from = "Yield") %>% 
  dplyr::select(Date,DE10M,	DE2M)

Dates <- as.yearmon(CH$Date)
input <- CH %>% dplyr::select(-Date) %>% xts(., order.by=Dates) 
turb1 <- OSturbulence(X=input, norm=TRUE, rolling=TRUE, roll.obs=60, use="pairwise.complete.obs")
turb2 <- turb1$turb %>% as_tibble() %>% add_column(Date=as.Date(Dates)) %>% dplyr::select(Date, turb)
turb.grad <- turb1$turb.grad %>% as_tibble() %>% add_column(Date=as.Date(Dates))
